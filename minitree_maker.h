...
// Fixed size dimensions of array or collections stored in the TTree if any.
   ...
   minitree_maker( const char* infilename = "input.root" );
   virtual ~minitree_maker();
   virtual Int_t    Cut(Long64_t entry);
   virtual Int_t    GetEntry(Long64_t entry);
   virtual Long64_t LoadTree(Long64_t entry);
   virtual void     Init(TTree *tree);
   virtual void     Loop( bool verbose = true, int maxEvt = 10 );
...
#ifdef minitree_maker_cxx
minitree_maker::minitree_maker( const char* infilename ) : fChain(0) 
{
...
minitree_maker::~minitree_maker()
{
   if (!fChain) return;
   delete fChain->GetCurrentFile();
}

Int_t minitree_maker::GetEntry(Long64_t entry)
{
// Read contents of entry.
   if (!fChain) return 0;
   return fChain->GetEntry(entry);
}
Long64_t minitree_maker::LoadTree(Long64_t entry)
{
...
void minitree_maker::Init(TTree *tree)
{
...
void minitree_maker::Show(Long64_t entry)
{
...
Int_t minitree_maker::Cut(Long64_t entry)
{
...
#endif // #ifdef minitree_maker_cxx
